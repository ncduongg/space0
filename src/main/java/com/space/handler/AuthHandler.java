...
public class AuthHandler {
  ...
  public static void verifyAuth(RoutingContext rc){
    ...
    if(token == null || token.isEmpty()){
      String cookie = rc.request().getHeader("Cookie");
      if (cookie == null || cookie.isBlank()) {
        jResponse.put("response_code", "002").put("status", "400").put("message", "Thông tin gửi lên không hợp lệ");
  ...
  public static void register(RoutingContext rc) {
    try {
      ...
      String userid = "USER_" + Base64.getUrlEncoder().withoutPadding().encodeToString(Util.parseHexBinary(UUID.randomUUID().toString().replaceAll("-", "")));
  ...
  public static void login(RoutingContext ctx) {
    try {
      ...
      if (username == null || password == null || username.isBlank() || password.isBlank()) {
        JsonObject bodyRes = new JsonObject().put("status", "400").put("response_code", "002").put("message", "Thông tin gửi lên không hợp lệ");
